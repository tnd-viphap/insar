...
class ComSAR_Prep:
    def __init__(self, master_date, data_dir, da_thresh=None, rg_patches=1, az_patches=1, 
        ...
        """
        Initialize ComSAR preparation class
        ...
        self.log_file = log_dir / f"comsar_prep_{timestamp}.log"
    ...
    def run(self):
        ...
        with open(self.work_dir / "calamp.in", 'w') as f:
            for file in glob.glob(f"{self.data_dir}/*slc/*.csar"):
                file = file.replace('\\', '/')
                f.write(f"{file}\n")
